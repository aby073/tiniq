...
const PREMIUM_BACKGROUND_PROMPT = "Premium fonni olib tashlash: mahsulotni aynan o'zidek saqla, asl fon, pol va ortiqcha buyumlarni butunlay olib tashla. Mahsulotni toza oq studiya fonida markazga joylashtir, yumshoq professional yorug'lik ber va mahsulot tagida tabiiy mayin soya qoldir. Minimalistik, yuqori sifatli katalog fotosi."
...
export function SellerWorkspace() {
  ...
  const [studioTab, setStudioTab] = useState<'create' | 'catalog' | 'copy'>('create')
  const [quality, setQuality] = useState<'standard' | 'pro'>('pro')
  const [batchMode, setBatchMode] = useState(false)
  const [brandKit, setBrandKit] = useState(false)
  const isRu = language === 'ru'
  const labels = isRu ? {
    history: 'История', works: 'Последние 20 работ', clear: 'Удалить всё', analyze: 'Анализировать товар', create: 'Создать фото', mode: 'Режим работы', standard: 'Стандарт', pro: 'Pro качество', batch: 'Пакет из 4', brand: 'Бренд-кит', highAccuracy: 'Высокая точность', neutralStyle: 'Нейтральный стиль', catalog: 'Каталог Uzum', catalogDesc: 'Белый фон для маркетплейса', copy: 'Текст и анализ', copyDesc: 'SEO-описание и продающий текст', upload: 'Фото товара', ready: 'AI готов', formats: 'PNG, JPG · 10 МБ', choose: 'Выберите фото или перетащите его сюда', chooseDesc: 'Снимите товар максимально чётко и при хорошем освещении', style: 'Стиль фото', catalogStyle: 'Стиль каталога Uzum Market', whiteShadow: 'Белый фон и мягкая тень', premium: 'Премиум-фон', result: 'Результат появится здесь', resultDesc: 'Загрузите фото и получите AI-описание или креатив для товара', generated: 'Созданное фото', description: 'Готовое описание', remove: 'Удалить фото', errorUpload: 'Пожалуйста, выберите файл изображения', uploadError: 'Ошибка загрузки', actionError: 'Не удалось выполнить действие', promptPlaceholder: 'Например: на мраморной подставке с тёплым студийным светом', ideas: ['Мраморная подставка и тёплый мягкий свет', 'Среди зелёных листьев и капель воды', 'Драматичный золотой свет на чёрном камне', 'Минималистичный белый фон с мягкой тенью'], ratios: ['Квадрат', 'Вертикальный', 'Горизонтальный', 'Оригинал']
  } : {
    history: 'Tarix', works: 'Oxirgi 20 ta ishlar', clear: 'Barchasini o‘chirish', analyze: 'Mahsulotni tahlil qilish', create: 'Foto yaratish', mode: 'Ish rejimi', standard: 'Standard', pro: 'Pro sifat', batch: 'Batch 4 ta', brand: 'Brand kit', highAccuracy: 'Yuqori aniqlik', neutralStyle: 'Neytral uslub', catalog: 'Uzum katalog', catalogDesc: 'Marketplace uchun tayyor oq fon', copy: 'Matn va tahlil', copyDesc: 'SEO opisaniya va sotuv copy', upload: 'Mahsulot rasmi', ready: 'AI tayyor', formats: 'PNG, JPG · 10MB', choose: 'Rasmni tanlang yoki shu yerga tashlang', chooseDesc: 'Mahsulotni imkon qadar aniq va yorug‘ joyda suratga oling', style: 'Foto uslubi', catalogStyle: 'Uzum Market katalog uslubi', whiteShadow: 'Oq fon va biroz soya uslubi', premium: 'Premium fon', result: 'Natija shu yerda ko‘rinadi', resultDesc: 'Rasm yuklang va mahsulotingiz uchun AI tavsif yoki kreativ oling', generated: 'Yaratilgan foto', description: 'Tayyor opisaniya', remove: 'Rasmni o‘chirish', errorUpload: 'Iltimos, rasm faylini tanlang', uploadError: 'Yuklashda xatolik', actionError: 'Amal bajarilmadi', promptPlaceholder: 'Masalan: marmar poydevor ustida, iliq studiya yorug‘ligi bilan', ideas: PROMPT_IDEAS, ratios: ['Kvadrat', 'Vertikal', 'Gorizontal', 'Original']
  }
  ...
  const runAction = async () => {
    ...
    try {
      ...
      const enhancedPrompt = [prompt, quality === 'pro' ? 'Ultra high detail, premium commercial product photography, accurate materials and clean edges.' : 'Fast clean commercial product photography.', brandKit ? 'Keep a consistent refined brand system with warm amber accents and premium editorial styling.' : '', batchMode ? 'Prepare a consistent visual direction suitable for a four-image product set.' : ''].filter(Boolean).join(' ')
      const body = mode === 'analyze' ? { imageUrl: uploadedUrl } : { prompt: enhancedPrompt, imageUrl: uploadedUrl, aspectRatio }
  ...
  return <section id="studio" className="relative isolate scroll-mt-20 overflow-hidden border-t border-border bg-secondary/20"><Starfield /><div className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-24">
    <div className="mb-10 flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs font-semibold text-primary"><WandSparkles className="h-3.5 w-3.5" /> SkySolve Studio <span className="rounded-full bg-primary/15 px-2 py-0.5 text-[10px] uppercase tracking-wider">Pro</span></div><h2 className="text-balance text-3xl font-semibold tracking-tight sm:text-4xl">{language === 'ru' ? 'Умный помощник по товарам для Uzum' : 'Uzum uchun aqlli mahsulot yordamchisi'}</h2><p className="mt-3 max-w-2xl leading-relaxed text-muted-foreground">{language === 'ru' ? 'Загрузите фото — SkySolve распознает товар, найдёт преимущества и подготовит описание. Создайте рекламное фото или сохраните результат.' : 'Rasmni yuklang — SkySolve mahsulotni taniydi, foydalarini topadi va tayyor opisaniya yozadi. Reklama fotosini yarating yoki ishlarini saqlang.'}</p></div><div className="flex gap-2"><button onClick={() => setShowHistory(!showHistory)} className="inline-flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-sm font-semibold hover:bg-secondary"><Settings2 className="h-4 w-4" /> {history.length > 0 ? `${labels.history} (${history.length})` : labels.history}</button><div className="flex rounded-xl border border-border bg-card p-1"><button onClick={() => setMode('analyze')} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${mode === 'analyze' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}><ScanSearch className="h-4 w-4" /> {t('analyze')}</button><button onClick={() => setMode('create')} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${mode === 'create' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}><Sparkles className="h-4 w-4" /> Foto yaratish</button></div></div></div>

    <div className="mb-6 grid gap-3 rounded-2xl border border-border bg-card/80 p-2 shadow-sm sm:grid-cols-2"><button type="button" onClick={() => { setStudioTab('catalog'); setMode('create'); setPrompt(UZUM_MARKET_PROMPT); setAspectRatio('3:4') }} className={`rounded-xl px-4 py-3 text-left transition-colors ${studioTab === 'catalog' ? 'bg-primary/12 ring-1 ring-primary/30' : 'hover:bg-background'}`}><span className="block text-sm font-semibold"><Store className="mr-1 inline h-4 w-4 text-primary" /> {labels.catalog}</span><span className="mt-1 block text-xs text-muted-foreground">{labels.catalogDesc}</span></button><button type="button" onClick={() => { setStudioTab('copy'); setMode('analyze') }} className={`rounded-xl px-4 py-3 text-left transition-colors ${studioTab === 'copy' ? 'bg-primary/12 ring-1 ring-primary/30' : 'hover:bg-background'}`}><span className="block text-sm font-semibold">{labels.copy}</span><span className="mt-1 block text-xs text-muted-foreground">{labels.copyDesc}</span></button></div><div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-background/70 px-4 py-3"><div className="flex flex-wrap items-center gap-2 text-xs"><span className="font-semibold text-muted-foreground">{labels.mode}</span><button type="button" onClick={() => setQuality('standard')} className={`rounded-lg border px-3 py-1.5 ${quality === 'standard' ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}>{labels.standard}</button><button type="button" onClick={() => setQuality('pro')} className={`rounded-lg border px-3 py-1.5 ${quality === 'pro' ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}>{labels.pro}</button><button type="button" onClick={() => setBatchMode(!batchMode)} className={`rounded-lg border px-3 py-1.5 ${batchMode ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}>{labels.batch}</button><button type="button" onClick={() => setBrandKit(!brandKit)} className={`rounded-lg border px-3 py-1.5 ${brandKit ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}>{labels.brand}</button></div><span className="text-xs text-muted-foreground">{quality === 'pro' ? (isRu ? 'Высокая точность' : labels.highAccuracy) : (isRu ? 'Быстрый результат' : 'Tezkor natija')} · {brandKit ? (isRu ? 'Фирменный стиль включён' : 'Brend uslubi yoqilgan') : (isRu ? 'Нейтральный стиль' : labels.neutralStyle)}</span></div>
