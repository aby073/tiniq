...
export function SellerWorkspace() {
  ...
  return <section id="studio" className="relative isolate scroll-mt-20 overflow-hidden border-t border-border bg-secondary/20"><Starfield /><div className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-24">
    ...
    <div className="mb-6 grid gap-3 rounded-2xl border border-border bg-card/80 p-2 shadow-sm sm:grid-cols-3"><button type="button" onClick={() => { setStudioTab('create'); setMode('create') }} className={`rounded-xl px-4 py-3 text-left transition-colors ${studioTab === 'create' ? 'bg-primary/12 ring-1 ring-primary/30' : 'hover:bg-background'}`}><span className="block text-sm font-semibold">AI fotosurat</span><span className="mt-1 block text-xs text-muted-foreground">Sahna yarating va reklama qiling</span></button><button type="button" onClick={() => { setStudioTab('catalog'); setMode('create'); setPrompt(UZUM_MARKET_PROMPT); setAspectRatio('3:4') }} className={`rounded-xl px-4 py-3 text-left transition-colors ${studioTab === 'catalog' ? 'bg-primary/12 ring-1 ring-primary/30' : 'hover:bg-background'}`}><span className="block text-sm font-semibold"><Store className="mr-1 inline h-4 w-4 text-primary" /> Uzum katalog</span><span className="mt-1 block text-xs text-muted-foreground">Marketplace uchun tayyor oq fon</span></button><button type="button" onClick={() => { setStudioTab('copy'); setMode('analyze') }} className={`rounded-xl px-4 py-3 text-left transition-colors ${studioTab === 'copy' ? 'bg-primary/12 ring-1 ring-primary/30' : 'hover:bg-background'}`}><span className="block text-sm font-semibold">Matn va tahlil</span><span className="mt-1 block text-xs text-muted-foreground">SEO opisaniya va sotuv copy</span></button></div><div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-background/70 px-4 py-3"><div className="flex flex-wrap items-center gap-2 text-xs"><span className="font-semibold text-muted-foreground">Ish rejimi</span><button type="button" onClick={() => setQuality('standard')} className={`rounded-lg border px-3 py-1.5 ${quality === 'standard' ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}>Standard</button><button type="button" onClick={() => setQuality('pro')} className={`rounded-lg border px-3 py-1.5 ${quality === 'pro' ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}>Pro sifat</button><button type="button" onClick={() => setBatchMode(!batchMode)} className={`rounded-lg border px-3 py-1.5 ${batchMode ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}>Batch 4 ta</button><button type="button" onClick={() => setBrandKit(!brandKit)} className={`rounded-lg border px-3 py-1.5 ${brandKit ? 'border-primary bg-primary/10 text-primary' : 'border-border'}`}>Brand kit</button></div><span className="text-xs text-muted-foreground">{quality === 'pro' ? 'Yuqori aniqlik' : 'Tezkor natija'} · {brandKit ? 'Brend uslubi yoqilgan' : 'Neytral uslub'}</span></div>
